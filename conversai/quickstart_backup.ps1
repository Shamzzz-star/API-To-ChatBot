...
if (!(Test-Path "backend")) {
    Write-Host "  âœ— Backend folder not found! Are you in the conversai directory?" -ForegroundColor Red
    exit 1
}
Write-Host "  âœ“ Directory structure looks good" -ForegroundColor Green

# Check for .env file
Write-Host "`n[3/6] Checking environment configuration..." -ForegroundColor Green
if (!(Test-Path ".env")) {
    Write-Host "  â„¹ .env file not found. Creating from template..." -ForegroundColor Yellow
    Copy-Item ".env.example" ".env"
    Write-Host "  âœ“ Created .env file" -ForegroundColor Green
    Write-Host ""
    Write-Host "  âš  IMPORTANT: Edit .env and add your GROQ_API_KEY!" -ForegroundColor Red
    ...
    if ($envContent -match "GROQ_API_KEY=gsk_") {
        Write-Host "  âœ“ GROQ_API_KEY appears to be configured" -ForegroundColor Green
    } else {
        Write-Host "  âš  GROQ_API_KEY may not be configured correctly" -ForegroundColor Yellow
...
if ($testResult -eq "SUCCESS") {
    Write-Host "  âœ“ All dependencies working correctly" -ForegroundColor Green
} else {
...
Write-Host "  âœ“ Setup Complete! ConversAI is ready to run." -ForegroundColor Green
Write-Host ("=" * 60) -ForegroundColor Green
Write-Host ""
